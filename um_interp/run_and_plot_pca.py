...
@torch.inference_mode()
def plot_attn_pca_single(model, config, test_dataset, a, c, shifts=0, start_pos=32, base_save_path: str | None = None):
    head_dim = config.n_embd // config.n_head
    ...
    for i, (id, m) in enumerate(model.named_modules()):
        ...
        if isinstance(m, MLP):
            mlp_hooks.append(Hook(m, backward=False))
    
    # Perform Forward Pass
    test_dataset = torch.roll(test_dataset, shifts=shifts, dims=-1)[:, :config.context_len]
    ...
    for i, (id, m) in enumerate(model.named_modules()):
        ...
        if isinstance(m, CausalSelfAttention):
            ...
            outputs = outputs[:, :, start_pos, :].contiguous() # (B, n_head, n_embd)
            annotate_sample = x[:, start_pos]  # (B,)
            print(annotate_sample.shape, outputs.shape)
            
            # Attention Map Plotting
            fig, axs = plt.subplots(1, config.n_head, figsize=(5 * config.n_head + 1, 5), constrained_layout=True)
            for head_id, ax in enumerate(axs):
                # Each head plot one sub-figure
                head_outputs = outputs[:, head_id, :]
                
                # Concat features from attention output
                concat_features = torch.zeros(outputs.size(0), outputs.size(-1), device=outputs.device, dtype=outputs.dtype)  # (B, spec_dim * n_embd)
                for sample_id in range(outputs.size(0)):
                    concat_features[sample_id] = head_outputs[sample_id, :]
                
                components = pca(concat_features, components=(0, 1))  # (2, n_embd)
                
                for sample_id in range(head_outputs.size(0)):
                    cord_x = (components[0, :] @ head_outputs[sample_id, :]).item()
                    cord_y = (components[1, :] @ head_outputs[sample_id, :]).item()
                    ax.plot(cord_x, cord_y, alpha=0)
                    ax.annotate(f'{annotate_sample[sample_id].item()}', (cord_x, cord_y), color=custom_cmap.colors[sample_id], fontsize=5)
            ...
            if base_save_path is not None:
                fig.savefig(base_save_path + f'attn_pca_a{a}c{c}_shifts{shifts}_l{attn_layer_id + 1}.pdf', format='pdf', dpi=dpi)
            else:
                plt.show()
            plt.close()
        ...
        # MLP Layers
        if isinstance(m, MLP):
            features = mlp_hooks[mlp_layer_id].output.cpu().to(dtype=torch.float32)
            feature_plot = features[:, start_pos, :]  # (B, n_embd)
            annotate_sample = x[:, start_pos]# (B,)
            
            components = pca(feature_plot, components=(0, 1))  # (2, n_embd)
            
            fig, ax = plt.subplots(1, 1, figsize=(5, 5), constrained_layout=True)
            for sample_id in range(feature_plot.size(0)):
                cord_x = (components[0, :] @ feature_plot[sample_id, :]).item()
                cord_y = (components[1, :] @ feature_plot[sample_id, :]).item()
                ax.plot(cord_x, cord_y, alpha=0)
                ax.annotate(f'{annotate_sample[sample_id].item()}', (cord_x, cord_y), color=custom_cmap.colors[sample_id], fontsize=5)
                
            ax.set_title(f'Layer: {mlp_layer_id + 1}')
            ax.set_ylabel('PCA component 2')
            ax.set_xlabel('PCA component 1')
            ...
            if base_save_path is not None:
                fig.savefig(base_save_path + f'MLP_pca_a{a}c{c}_shifts{shifts}_l{mlp_layer_id + 1}.pdf', format='pdf', dpi=dpi)
            else:
                plt.show()
            plt.close()
...
parser.add_argument('--plots_dir', type = str, default = 'plots/lcg/multiple_ps/pca')
parser.add_argument('--save_sequences', type = str, default = 'False')
parser.add_argument('--save_checkpoint', type = str, default = 'False')
# Other
parser.add_argument('--shifts', type = int, default = 32) # position of 1 to p_eval numbers in the sequence
...
seq_collections = lcg_vectorized_with_fixed_seed(p = chosen_p, length = chosen_p + 1, a_list = a_list, c_list = c_list) # (all_ac_pairs, p, seq_len)

pairs = [(i, j) for j in range(1, 10) for i in range(0, j)]
for pair in pairs:
    plot_embd_pca(model, components=pair, base_save_path=plot_path)

for c_id, c in enumerate(c_list):
    for a_id, a in enumerate(a_list):
        print(a, c)
        test_dataset = seq_collections[c_id, a_id, :, :]
        plot_attn_pca_single(model, config, test_dataset, a, c, shifts=config.shifts, start_pos=0, base_save_path=plot_path)
